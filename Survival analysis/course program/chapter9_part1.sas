...
See Section 1.3
Data can be read in free format. The variables represented in the data set are as follows: 
g--Disease Group 
      1-ALL 
      2-AML Low Risk 
      3-AML High Risk 
...
1 -- Death Indicator 
      1-Dead 0-Alive 
2 -- Relapse Indicator 
      1-Relapsed, 0-Disease Free 
3--Disease Free Survival Indicator 
      1-Dead Or Relapsed, 0-Alive Disease Free) 
TA--Time To Acute Graft-Versus-Host Disease
A--Acute GVHD Indicator 
      1-Developed Acute GVHD 0-Never Developed Acute GVHD) 
TC--Time To Chronic Graft-Versus-Host Disease 
C--Chronic GVHD Indicator 
      1-Developed Chronic GVHD 0-Never Developed Chronic GVHD 
TP--Time To Return of Platelets to Normal Levels 
P--Platelet Recovery Indicator 
      1-Platelets Returned To Normal, 0-Platelets Never Returned to Normal 
...
Z9--Hospital 
      1-The Ohio State University, 2-Alferd , 3-St. Vincent, 4-Hahnemann 
Z10--MTX Used as a Graft-Versus-Host- Prophylactic 1-Yes 0-No 
***/

data chapter9;
input group death_t d_free_t death_i relap_i d_free_i aGVHD_t aGVHD_i cGVHD_t cGVHD_i plate_t plate_i 
                  p_age d_age p_gender d_gender p_CMV_i d_CMV_i wait_t FAB_i hospital MTX_i;
cards;
...
proc phreg data=mydata1;
  model d_free_t*d_free_i(0) = aGVHD_t group2 group3 ;
  
run;

proc phreg data=mydata1;
  model d_free_t*d_free_i(0) = Z_a group2 group3 ;
  if ( d_free_t >= aGVHD_t & aGVHD_i = 1  ) then  Z_a = 1; 
  else  Z_a = 0;
  test Z_a;
run;

proc phreg data=mydata1;
  model d_free_t*d_free_i(0) = Z_c group2 group3 ;
  if ( d_free_t >= cGVHD_t & cGVHD_i=1  ) then  Z_c = 1; 
  else  Z_c = 0;
  test Z_c;
run;

proc phreg data=mydata1;
  model d_free_t*d_free_i(0) = Z_p group2 group3 ;
  if ( d_free_t >= plate_t & plate_i = 1  ) then  Z_p = 1; 
  else  Z_p = 0;
  test Z_p;
run;
